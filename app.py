...
def draw_mollier(T_a, RF_a, T_r, RF_i, p_pa=101325.0):
    ...
    for h_val in np.arange(-20, 130, 10):
        ...
        if mask.sum() > 1:
            ax.plot(x_plot[mask], T_h[mask],
                    color="#CFE8E5", lw=0.6, alpha=0.6, zorder=1)
    ...
    for rf_val, col in zip(rf_levels, rf_colors):
        ...
        if vis.sum() > 1:
            lw = 2.4 if rf_val == 100 else (
                1.3 if rf_val % 20 == 0 else 0.7
            )
            ax.plot(x_arr[vis], T_plot[vis],
                    color=col, lw=lw, alpha=0.85, zorder=2)
            for tgt in [35, 30, 25, 20, 15, 10]:
                ...
                if np.isfinite(lx) and 0.3 < lx < X_MAX - 0.5:
                    ax.text(lx + 0.15, lT, f"{rf_val}%",
                            ...
                            fontweight="bold" if rf_val in [80, 100] else "normal",
                            va="center", ha="left",
    ...
    _lbl(x_a, T_a,
         ...
         f"x = {x_a:.2f} g/kg   h = {h_a:.1f} kJ/kg",
    ...
    _lbl(x_i, T_r,
         ...
         f"x = {x_i:.2f} g/kg   h = {h_i:.1f} kJ/kg",
    ...
    handles = [
        ...
        Line2D([0],[0], marker="o", color="w", lw=0,
               markerfacecolor=C["außen"], markersize=10, label="① Außenluft"),
        ...
        Line2D([0],[0], marker="s", color="w", lw=0,
               markerfacecolor=C["t_krit"],markersize=9,  label="⑤ T_krit"),
        mpatches.Patch(color="none", label=" "),
        mpatches.Patch(color="none", label="─── Prozesse ───"),
        Line2D([0],[0], color=C["sat"],    lw=2.5, label="Sättigungslinie"),
        Line2D([0],[0], color=C["p_auf"],  lw=2.0, label="① → ② Aufheizung"),
        Line2D([0],[0], color=C["p_feu"],  lw=2.0, label="② → ③ Befeuchtung"),
        Line2D([0],[0], color=C["tau"],    lw=1.6, ls="--", label="③ → ④ Abkühlung"),
        Line2D([0],[0], color=C["t_krit"], lw=2.0, ls="--", label="⑤ Schimmelgrenze"),
    ]
    ...
    ax.set_title(
        "Mollier  h-x-Diagramm  —  Feuchte Luft   (p = 1013,25 hPa)",
...
.footer {
    ...
    font-size: 0.8em; margin-top: 24px;
...
# ══════════════════════════════════════════════════════
st.markdown(
    "<h1 style='text-align:center; color:#1A252F;'>"
...
st.markdown(
    ...
    f"<b style='color:#CA6F1E; font-size:1.1em;'>"
    f"⑤ T_krit = {T_surf:.1f} °C</b><br><br>"
    f"Keine Oberfläche darf unter <b>{T_surf:.1f} °C</b> abkühlen.<br>"
    f"Darunter → φ_Oberfläche ≥ 80 % → <b>Schimmelgefahr!</b><br><br>"
    f"<small>Taupunkt: {T_tau:.1f} °C  |  "
    f"T_krit liegt immer über dem Taupunkt.</small>"
    f"</div>",
    unsafe_allow_html=True
)
...
if ist_norm:
    st.markdown(
        ...
        f"<b style='color:#1A7A42;'>"
        f"✅ Norm-Randbedingungen — DIN 4108-2 Beiblatt 2</b><br><br>"
        f"T_i = {NORM_T_I:.0f} °C  |  "
        f"T_e = {NORM_T_E:.0f} °C  |  "
        f"φ_i = {NORM_RF_I:.0f} %<br><br>"
        f"T_krit = <b>{T_surf:.1f} °C</b>  |  "
        f"f_Rsi,min = <b>{f_rsi_min:.3f}</b><br><br>"
        f"<b>Das Bauteil muss f_Rsi ≥ 0,70 aufweisen.</b>"
        f"</div>",
        unsafe_allow_html=True
    )
else:
    ...
    st.markdown(
        f"<div class='abw-box'>"
        f"<b style='color:#A04000;'>⚠️ Abweichende Randbedingungen</b>"
        f"<br><br>{abw_html}<br><br>"
        f"T_krit = <b>{T_surf:.1f} °C</b>  |  "
        f"f_Rsi,min = <b>{f_rsi_min:.3f}</b>"
        f"</div>",
        unsafe_allow_html=True
    )
